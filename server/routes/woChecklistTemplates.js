// Admin-managed, global checklist questions for Preventive Maintenance
// work orders. Copied onto every new PM-type work order at creation time
// (see workOrders.js POST /).
...
router.get('/', async (req, res) => {
  ...
  const [rows] = await pool.query(`SELECT id, question, sort_order, is_active FROM wo_checklist_templates ${where} ORDER BY sort_order, id`);
...
router.put('/:id', authorizeModule('wo_checklist', 'manage'), async (req, res) => {
  ...
  await pool.query(`UPDATE wo_checklist_templates SET ${sets.join(', ')} WHERE id = ?`, params);
  res.json({ ok: true });
});

router.delete('/:id', authorize('Administrator'), async (req, res) => {
  await pool.query('DELETE FROM wo_checklist_templates WHERE id = ?', [req.params.id]);
  res.json({ ok: true });
});
